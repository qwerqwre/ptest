					...
					<fieldset>
							...
							<tbody>
								<c:forEach items="${list}" var="row">
									<tr>
										...
										<td class="tleft">
											<span class="bold">
												<a href="<c:url value='/board/read.do?typeSeq=${row.type_seq}&boardSeq=${row.board_seq}' />">
													${row.title}
												</a>
											</span>
										</td>
						...
						<div class="paginate">
							<c:if test="${pageBlockStart > pageBlockSize}">
								<a class="pre" href="<c:url value='/board/list.do?typeSeq=${typeSeq}&currentPage=${pageBlockStart - 10}&searchType=${searchType}&searchText=${searchText}'/>">이전페이지</a>
							</c:if>
							<c:forEach begin="${pageBlockStart}" end="${pageBlockEnd}" step="1" var="pageNo">
								<c:choose>
									...
									<c:otherwise>
										<a href="<c:url value='/board/list.do?typeSeq=${typeSeq}&currentPage=${pageNo}&searchType=${searchType}&searchText=${searchText}' />">${pageNo}</a>
									</c:otherwise>
								</c:choose> 
							</c:forEach>
							
							<c:if test="${pageBlockEnd != totalPageCnt}">
								<a class="next" href="<c:url value='/board/list.do?typeSeq=${typeSeq}&currentPage=${pageBlockEnd + 1}&searchType=${searchType}&searchText=${searchText}'/>">다음페이지</a>
							</c:if>
						</div>
						<!--//paginate end -->
